(function () {
  ...
  function money(n, ask) {
    if (n === 0) return ask ? "ask" : "by wt";
  ...
  function add(item) {
    ...
    else cart.push({ id: item.id, name: item.name, price: item.price, ask: item.ask, qty: 1 });
    render();
  }
  ...
  function lineLabel(item) {
    if (item.price === 0) return money(0, item.ask);
    return money(item.price * item.qty, item.ask);
  }
  ...
  function render() {
    ...
    if (totalEl) {
      totalEl.hidden = !has;
      totalEl.textContent = "Weight & price confirmed by kitchen";
    }
    renderLines();
    markAdds();
    renderDock();
  ...
  function message(data) {
    const rows = cart.map(function (item) {
      return "• " + item.name + " × " + item.qty + " — " + lineLabel(item);
    });
    const lines = [
      "Pre-order for Mr.Jeff",
      "",
      "Name: " + data.name,
      "WhatsApp: " + data.phone,
      "People: " + data.people,
  ...
  document.addEventListener("click", function (e) {
    ...
    if (addBtn) {
      add({
        ...
        ask: addBtn.getAttribute("data-ask") === "1" || addBtn.getAttribute("data-id").indexOf("side-") === 0
      });
      return;
    }
