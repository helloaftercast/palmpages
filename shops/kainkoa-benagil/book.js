(function () {
  ...
  function greet() {
    return isPt()
      ? "Olá Kai'n Koa, queria reservar um kayak na Praia de Benagil."
      : "Hi Kai'n Koa, I'd like to book a kayak at Benagil beach.";
  }
  ...
  function firstError() {
    ...
    if (!slotEl || !slotEl.value) {
      return { el: slotEl, msg: isPt() ? "Escolham nascer ou pôr do sol." : "Pick sunrise or sunset." };
    }
    return null;
  }
  ...
  function fillSlots() {
    ...
    slotEl.innerHTML = "";
    var labels = {
      sunrise: isPt() ? "Nascer do sol" : "Sunrise",
      sunset: isPt() ? "Pôr do sol" : "Sunset"
    };
    SLOTS.forEach(function (slot) {
      var opt = document.createElement("option");
      opt.value = slot;
      opt.textContent = labels[slot] || slot;
  ...
  function message(data) {
    ...
    var lines = pt ? [
      "Reserva Kai'n Koa — kayak nascer/pôr do sol, Praia de Benagil",
      "",
      "Nome: " + data.name,
      "WhatsApp: " + data.phone,
      "Pessoas: " + data.people,
      ...
      "Booking for Kai'n Koa — sunrise/sunset kayak, Benagil beach",
      "",
      "Name: " + data.name,
      "WhatsApp: " + data.phone,
      "People: " + data.people,
  ...
  function prepareChat() {
    if (!chatLayer) return;
    var pt = isPt();
    chatLayer.setAttribute("data-name", "Kai'n Koa Adventures");
    chatLayer.setAttribute(
      "data-reply",
      pt
        ? "Recebido. Confirmamos nascer/pôr do sol e o mar na véspera."
        : "Got it — we confirm sunrise or sunset and the sea the night before."
    );
