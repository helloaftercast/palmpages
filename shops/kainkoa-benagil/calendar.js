(function () {
  ...
  function slotLabel(slot, taken) {
    ...
    if (taken) return isPt() ? name + " · ocupado" : name + " · taken";
  ...
  function confirmLine() {
    return isPt()
      ? (root.getAttribute("data-confirm-pt") || "Confirmem no WhatsApp.")
      : (root.getAttribute("data-confirm-en") || "Confirm on WhatsApp.");
  }
  ...
  function render() {
    ...
    if (statusEl) {
      if (!data.busy.length) {
        statusEl.textContent = isPt()
          ? (root.getAttribute("data-empty-pt") || "Deslizem os dias. Ainda não há saídas marcadas como ocupadas. Não é em tempo real.")
          : (root.getAttribute("data-empty-en") || "Scroll the days. None marked taken. Not live.");
      } else {
        statusEl.textContent = isPt()
          ? "Deslizem os dias. Cinzento = essa saída parece ocupada. Não é em tempo real."
    ...
    if (updatedEl) {
      ...
      var source = isPt() ? "Marcado à mão." : "Marked by hand.";
      updatedEl.textContent = when
        ? source + (isPt() ? " Última nota: " + when + " (Lisboa). " : " Last marked: " + when + " Lisbon. ") + confirmLine()
        : source + " " + confirmLine();
    }
  }
  ...
  function load() {
    ...
    fetch(local).then(function (res) { return res.json(); }).then(applyData).catch(function () {
      if (statusEl) {
        statusEl.textContent = isPt()
          ? "Não foi possível carregar as saídas marcadas."
