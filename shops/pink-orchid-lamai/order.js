(function () {
  ...
  function render() {
    ...
    if (totalEl) {
      totalEl.hidden = !has;
      const sum = total();
      totalEl.textContent = sum ? "Est. " + money(sum) : "Prices confirmed on WhatsApp";
    }
    renderLines();
    markAdds();
    renderDock();
  ...
  function fillSlots() {
    ...
    if (!slotEl.options.length) {
      const opt = document.createElement("option");
      opt.value = "";
      opt.textContent = "Too late to pre-order — kitchen closes 22:30";
      slotEl.appendChild(opt);
    } else if (keep) {
      slotEl.value = keep;
    }
  ...
  function message(data) {
    ...
    const sum = total();
    const lines = [
      "Pre-order for Pink Orchid Restaurant",
      "",
      "Name: " + data.name,
      "WhatsApp: " + data.phone,
      "People: " + data.people,
      ...
      rows.join("\n"),
      "",
      sum ? "Est. " + money(sum) : "Please confirm prices.",
