(function () {
  var MAIL = "info@seacanoe.com";
  var TEL = "66894451756";
  var TEL_OFFICE = "6676528841";
  var LEAD_MIN = 90;
  var SLOTS = ["morning", "sunset"];
  var PRICES = {
    day: 3500,
    self: 3750,
    sunset: 3750,
  ...
  function greet() {
    return isTh()
      ? "สวัสดี Sea Canoe Thailand อยากจองทัวร์คายัคครับ/ค่ะ"
      : "Hi Sea Canoe Thailand, I'd like to book a kayak tour.";
  }

  function mailHref(text) {
    var sub = isTh() ? "จองทัวร์ Sea Canoe" : "Sea Canoe booking";
  ...
  function officeHref() {
    return "tel:+" + TEL_OFFICE;
  }

  function channelHref(kind, text) {
    if (kind === "tel") return telHref();
    if (kind === "office") return officeHref();
    return mailHref(text);
  }
  ...
  function syncDock() {
    ...
    reserveDock.classList.add("is-mail");
  ...
  function firstError() {
    ...
    if (!slotEl || !slotEl.value) {
      return { el: slotEl, msg: isTh() ? "เลือกช่วง" : "Pick a launch." };
    }
    return null;
  }
  ...
  function slotLabel(value) {
    ...
    if (value === "sunset") return isTh() ? "พระอาทิตย์ตก" : "Sunset";
    return value;
  }

  function priceLines() {
    return cart.filter(function (item) { return item.price; }).map(function (item) {
      return itemName(item) + " " + money(item.price) + (isTh() ? " ต่อคน" : " per person");
    });
  }
  ...
  function message(data) {
    ...
    var th = isTh();
    var priced = priceLines();
    var lines = th ? [
      "จอง Sea Canoe Thailand — คายัค อ่าวพังงา",
      "",
      "ชื่อ: " + data.name,
      "ติดต่อ: " + data.phone,
      "จำนวนคน: " + data.people,
      ...
      "Booking for Sea Canoe Thailand — kayak, Phang Nga Bay",
      "",
      "Name: " + data.name,
      "Contact: " + data.phone,
      "People: " + data.people,
  ...
  function sendBooking() {
    ...
    if (window.playOrderChat) {
      window.playOrderChat({ text: text, href: mailHref(text), copy: true });
    } else {
      openChannels(false);
    }
    return true;
  ...
  function render() {
    ...
    if (totalEl) {
      var priced = cart.filter(function (item) { return item.price; });
      if (priced.length) {
        totalEl.hidden = false;
        totalEl.textContent = isTh()
          ? "ราคาต่อคนตามรายการ · ค่าอุทยานไม่รวม"
          : "Per person as listed · park fee extra";
      } else {
        totalEl.hidden = true;
        totalEl.textContent = "";
      }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (addBtn) {
      add({
        ...
        price: Number(addBtn.getAttribute("data-price") || PRICES[addBtn.getAttribute("data-id")] || 0)
      });
    }
