(function () {
  ...
  function greet() {
    return isTh() ? "สวัสดีครับ/ค่ะ ร้าน No.11 ขอจองโต๊ะหน่อย" : "Hi No.11, I'd like a table.";
  }
  ...
  function renderDock() {
    ...
    var head = isTh() ? "ออเดอร์ · " : "Order · ";
    var ask = isTh() ? "สอบถาม" : "ask";
  ...
  function fillSlots() {
    ...
    if (!slotEl.options.length) {
      ...
      opt.textContent = isTh()
        ? "สายเกินไปสำหรับวันนี้ — ร้านปิด 18:00"
        : "Too late to pre-order — we close at 18:00";
      slotEl.appendChild(opt);
    } else if (keep) {
      slotEl.value = keep;
    }
  ...
  function message(data) {
    ...
    var lines = th ? [
      "สั่งล่วงหน้า No.11 CAFE & BAR ละไม",
      "",
      "ชื่อ: " + data.name,
      chLabel + ": " + data.phone,
      "จำนวนคน: " + data.people,
  ...
  function prepareChat(ch) {
    if (!chatLayer) return;
    var th = isTh();
    chatLayer.setAttribute("data-name", "No.11 CAFE & BAR");
    if (ch === "line") {
      chatLayer.setAttribute(
        "data-reply",
        th
          ? "รับแล้วค่ะ เดี๋ยวยืนยันโต๊ะและราคาให้ — ข้อความคัดลอกไว้แล้ว วางใน LINE ได้เลย"
          : "Got it — we'll confirm your table and any ask-price items. Message copied; paste it in LINE."
      );
      ...
      chatLayer.setAttribute(
        "data-reply",
        th
          ? "รับแล้วค่ะ เดี๋ยวยืนยันโต๊ะและราคาให้ เจอกันที่ No.11 นะคะ"
          : "Got it — we'll confirm your table and any ask-price items. See you at No.11."
      );
